...
import { supabase, createUsuarioAccount, notifyNewAccount } from '@/lib/supabase'
  ...
  const addUsuario = useCallback(async (usuarioData: Omit<Usuario, 'id'>, options?: { 
    ...
    try {
      ...
      if (options?.createAccount) {
        try {
          console.log('Criando conta automática de usuário...')
          
          const accountResult = await createUsuarioAccount({
          ...
          console.log('Conta de usuário criada:', accountResult)
          
          const newUsuario = databaseToUsuario(accountResult.usuario)
          setUsuarios(prev => [newUsuario, ...prev])
          
          // Notificar usuário se solicitado
          if (options?.notifyUser) {
            await notifyNewAccount(usuarioData.email, accountResult.senhaTemporaria, 'usuario')
          }
          ...
          console.error('Erro ao criar conta de usuário:', accountError)
          throw accountError
        }
      } else {
