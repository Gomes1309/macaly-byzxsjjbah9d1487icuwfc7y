...
export function useUsuarios() {
  ...
  const addUsuario = useCallback(async (usuarioData: Omit<Usuario, 'id'>, options?: { 
    ...
    try {
      ...
      // Usar a estrutura real do banco
      const dbData = {
        nome: usuarioData.nome,
        email: usuarioData.email,
        senha_hash: senhaHash, // Senha hasheada de forma segura
        tipo_usuario: usuarioData.cargo || 'admin',
        ativo: usuarioData.status === 'ativo'
      }
      ...
      return {
        ...
        account: options?.createAccount ? { 
          created: true,
          senhaTemporaria: senhaTemporaria // Retornar senha temporária para envio por email
        } : null,
