...
export function useObrigacoes() {
  ...
  const retryWithBackoff = useCallback(async (fn: () => Promise<any>, maxRetries = 3, initialDelay = 1000) => {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.log(`useObrigacoes: Tentativa ${attempt}/${maxRetries} falhou:`, error?.message || error)
  ...
  const loadObrigacoes = useCallback(async () => {
    try {
      ...
      console.log('Carregando obrigações do Supabase...')
      ...
      const obrigacoesData = result || []
      const formattedObrigacoes = obrigacoesData.map(databaseToObrigacao)
      setObrigacoes(formattedObrigacoes)
      console.log(`✅ ${formattedObrigacoes.length} obrigações carregadas do Supabase`)
      
      // Backup local
      localStorage.setItem('obrigacoes_backup', JSON.stringify(formattedObrigacoes))
      
    } catch (err: any) {
      console.error('Erro ao carregar obrigações:', {
      ...
      try {
        const backupData = localStorage.getItem('obrigacoes_backup')
        if (backupData) {
          const obrigacoesBackup = JSON.parse(backupData).map((obrigacao: any) => ({
            ...obrigacao,
          ...
          setObrigacoes(obrigacoesBackup)
          console.log(`✅ ${obrigacoesBackup.length} obrigações carregadas do backup local`)
  ...
  }, [databaseToObrigacao, retryWithBackoff])
  ...
  const deleteObrigacao = useCallback(async (id: string) => {
    try {
      ...
      console.log('🗑️ Deletando obrigação:', id)
      
      // Buscar obrigação antes de deletar para logs
      const obrigacaoToDelete = obrigacoes.find(o => o.id === id)
      ...
      emitSync({
        type: 'obrigacoes_updated',
        action: 'delete',
        data: { id, obrigacao: obrigacaoToDelete }
      })
      
      // Limpar cache local
      localStorage.removeItem('obrigacoes_backup')
      
      console.log('✅ Obrigação deletada com sucesso')
