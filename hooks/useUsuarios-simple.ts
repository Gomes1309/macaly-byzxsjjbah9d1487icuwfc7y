...
export function useUsuariosSimple() {
  ...
  const addUsuario = async (userData: Omit<Usuario, 'id'>, options?: { 
    ...
    try {
      ...
      // Gerar senha temporária se for criar conta
      let senhaTemporaria = 'senha123'
      let emailResult: { success: boolean; message: string; simulated?: boolean } | null = null
      
      if (options?.createAccount) {
        senhaTemporaria = Math.random().toString(36).slice(-4) + Math.random().toString(36).slice(-4).toUpperCase()
          ...
          try {
            const emailResponse = await fetch('/api/send-welcome-email', {
              ...
              body: JSON.stringify({
                ...
                empresas: [] // Lista de empresas - pode ser expandido depois
              })
            })
            ...
            if (emailResult && !emailResult.success) {
              console.warn('⚠️ Erro no envio de email:', emailResult.message)
            }
          } catch (emailError) {
            console.error('❌ Erro ao chamar API de email:', emailError)
            emailResult = { success: false, message: 'Erro ao processar envio de email' }
          }
        }
      }
      ...
      return {
        ...
        email: emailResult || { success: false, message: 'Email não enviado' },
        message: (options?.createAccount && options?.notifyUser && emailResult?.success) 
