...
export function useDocumentos() {
  ...
  const loadDocumentos = async (retryCount = 0) => {
    console.log('useDocumentos: Carregando documentos...', retryCount > 0 ? `(tentativa ${retryCount + 1})` : '');
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (healthError) {
        ...
        if (retryCount < 2 && (
          ...
          console.warn(`useDocumentos: Erro de conectividade (${healthError.message}), tentando novamente em 2s...`);
          setTimeout(() => loadDocumentos(retryCount + 1), 2000);
          return;
        }
      ...
      if (retryCount < 2 && isConnectivityError) {
        console.warn(`useDocumentos: Tentativa ${retryCount + 1}/3 falhou (conectividade), tentando novamente em 2s...`);
        setTimeout(() => loadDocumentos(retryCount + 1), 2000);
        return;
      }
      ...
      if (errorMessage && errorMessage.includes('does not exist')) {
        console.warn('useDocumentos: Tabela não existe, retornando lista vazia');
        ...
        console.warn('useDocumentos: Problema de conectividade persistente, mantendo lista vazia');
        setDocumentos([]);
        setError(null);
      } else {
        // Apenas logar erros reais que não são de conectividade
        console.error('useDocumentos: Erro real detectado:', {
