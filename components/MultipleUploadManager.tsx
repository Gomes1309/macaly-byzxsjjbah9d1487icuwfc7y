...
interface FileWithMetadata {
  ...
  progress: number
  clienteId?: string
  manuallySelectedCompany?: { id: string; nome: string; cnpj: string } // Nova propriedade
  showCompanySelector?: boolean // Para controlar se mostra o seletor
}
...
export default function MultipleUploadManager() {
  ...
  // Estados para seleção manual de empresa
  const [showCompanySearchDialog, setShowCompanySearchDialog] = useState(false)
  const [selectedFileForCompany, setSelectedFileForCompany] = useState<string | null>(null)
  const [companySearchTerm, setCompanySearchTerm] = useState('')
  
  const fileInputRef = useRef<HTMLInputElement>(null)

  // Função para selecionar empresa manualmente
  const selectCompanyForFile = useCallback((fileId: string, company: { id: string; nome: string; cnpj: string }) => {
    setFiles(prev => prev.map(f => 
      f.id === fileId 
        ? { 
            ...f, 
            manuallySelectedCompany: company,
            clienteId: company.id,
            detectedCompany: company.nome,
            detectedCNPJ: company.cnpj,
    ...
    toast({
      ...
      description: `${company.nome} foi vinculada ao arquivo.`,
    })
  }, [toast])

  // Função para abrir seletor de empresa
  const openCompanySelector = useCallback((fileId: string) => {
    setSelectedFileForCompany(fileId)
    setShowCompanySearchDialog(true)
    setCompanySearchTerm('')
  }, [])

  // Função para filtrar empresas/clientes na busca
  const filteredCompanies = useCallback(() => {
    const searchLower = companySearchTerm.toLowerCase()
    
    // Combinar clientes e empresas em uma lista única
    const allCompanies = [
      // Clientes
      ...
      ...(empresas || []).map(empresa => ({
        ...
        cnpj: empresa.cnpj || 'Sem CNPJ',
        tipo: 'empresa' as const
      }))
    ]
    
    if (!searchLower) {
      return allCompanies.slice(0, 10) // Limitar a 10 resultados quando não há busca
    }
    
    return allCompanies.filter(company => 
      company.nome.toLowerCase().includes(searchLower) ||
      (company.cnpj && company.cnpj.replace(/\D/g, '').includes(searchLower.replace(/\D/g, '')))
    ).slice(0, 20) // Limitar a 20 resultados
  }, [companySearchTerm, clientes, empresas])
  ...
  const processSelectedFiles = useCallback((selectedFiles: FileList) => {
    ...
    const processedFiles: FileWithMetadata[] = Array.from(selectedFiles).map(file => {
      ...
      const fileWithMetadata: FileWithMetadata = {
        ...
        manuallySelectedCompany: undefined,
        showCompanySelector: false
      }
  ...
  const uploadSingleFile = useCallback(async (fileData: FileWithMetadata): Promise<void> => {
    ...
    return new Promise(async (resolve) => {
      let uniqueFileName = '' // Declarar aqui para usar em caso de erro
      ...
      try {
        ...
        uniqueFileName = `doc_${timestamp}_${randomId}.${fileExtension}`
        
        console.log('🔄 Fazendo upload real para Supabase Storage...')
        
        // Upload real para o Supabase Storage
        const { data: storageData, error: storageError } = await supabase.storage
          .from('documentos')
          .upload(uniqueFileName, fileData.file, {
            cacheControl: '3600',
            upsert: false
          })
        ...
        console.log('✅ Upload do arquivo concluído:', storageData.path)
        ...
        // Preparar dados do documento (usando apenas colunas que existem)
        const documentoData = {
          clienteId: fileData.clienteId,
          nomeArquivo: fileData.file.name,
          tipoDocumento: (fileData.detectedType || 'pessoal') as any,
          categoria: 'geral', // Valor padrão, coluna não existe no banco
          urlArquivo: fileUrl,
          tamanhoArquivo: fileData.file.size,
          dataUpload: new Date(),
          uploadedBy: 'Sistema Upload Múltiplo', // Valor padrão, coluna não existe no banco
          observacoes: `Upload automático via detecção inteligente. Empresa detectada: ${fileData.detectedCompany || 'Manual'}. Pasta: ${folderPath}. Arquivo: ${uniqueFileName}`
        }
        ...
        // Em caso de erro, tentar remover arquivo do storage
        if (uniqueFileName) {
          try {
            await supabase.storage
              .from('documentos')
              .remove([uniqueFileName])
            console.log('🧹 Arquivo removido do storage após erro:', uniqueFileName)
          } catch (cleanupError) {
            console.warn('⚠️ Não foi possível limpar arquivo após erro:', cleanupError)
          }
        }
  ...
  }, [addDocumento, useCustomFolder, customFolder, selectedFolder])
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Detecção automática */}
                        {fileData.detectedCompany && !(fileData.manuallySelectedCompany) && (
                        ...
                        {/* Empresa selecionada manualmente */}
                        {fileData.manuallySelectedCompany && (
                          <div className="flex items-center space-x-2 mt-2">
                            <Building2 className="w-4 h-4 text-blue-600" />
                            <span className="text-sm text-blue-700 font-medium">
                              {fileData.manuallySelectedCompany.nome}
                            </span>
                            ...
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => openCompanySelector(fileData.id)}
      ...
      <Dialog open={showCompanySearchDialog} onOpenChange={setShowCompanySearchDialog}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Busque e selecione a empresa para vincular ao documento
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            {/* Campo de Busca */}
            <div className="space-y-2">
              <Label htmlFor="company-search">Buscar por nome ou CNPJ</Label>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              <Label>Empresas Encontradas ({filteredCompanies().length})</Label>
              ...
              {empresasLoading ? (
                ...
                filteredCompanies().map((company) => (
                  <Card 
                    key={`${company.tipo}-${company.id}`}
                    className="cursor-pointer hover:shadow-md transition-shadow border-gray-200 hover:border-blue-300"
                    onClick={() => {
                      if (selectedFileForCompany) {
                        selectCompanyForFile(selectedFileForCompany, company);
                  ...
                  >
                    <CardContent className="p-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex items-center space-x-2">
                          <Badge variant={company.tipo === 'cliente' ? 'default' : 'secondary'}>
                            {company.tipo === 'cliente' ? 'Cliente' : 'Empresa'}
                          </Badge>
                        </div>
                      </div>
                    </CardContent>
          ...
          <DialogFooter>
            <Button 
              variant="outline" 
              onClick={() => setShowCompanySearchDialog(false)}
            >
              Cancelar
            </Button>
          </DialogFooter>
