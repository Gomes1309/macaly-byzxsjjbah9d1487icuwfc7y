...
export function CNPJSearch({ 
  ...
  const handleSearch = async (cnpj?: string) => {
    ...
    console.log('CNPJSearch: Starting search for:', searchCNPJ_value)
    
    if (!searchCNPJ_value) {
      console.log('CNPJSearch: Empty CNPJ, aborting search')
      return
    }
    
    try {
      const data = await searchCNPJ(searchCNPJ_value)
      console.log('CNPJSearch: Search completed, data:', data)
      
      if (data && onDataFound) {
        console.log('CNPJSearch: Calling onDataFound callback')
        onDataFound(data)
      }
    } catch (error) {
      console.error('CNPJSearch: Search failed:', error)
    }
  }
