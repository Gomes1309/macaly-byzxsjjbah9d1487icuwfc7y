  ...
  const handleSave = async () => {
    try {
      ...
      if (editingVinculo) {
        ...
        const novoResponsavel = await addResponsavelToCliente(clienteId, {
        ...
        console.log('Novo responsável/vínculo adicionado com sucesso:', novoResponsavel)
        
        // Aguardar um pouco para garantir que a operação foi persistida
        await new Promise(resolve => setTimeout(resolve, 1000))
        
        // Mostrar mensagem de sucesso específica
        console.log('✅ Responsável criado e persistido com sucesso!')
      }
      ...
      // Verificar se é erro de vínculo duplicado ou outro erro conhecido
      const errorMessage = err instanceof Error ? err.message : 'Erro ao salvar responsável'
      
      if (errorMessage.includes('já está vinculado')) {
        setFormError('Este responsável já possui acesso a esta empresa. Verifique a lista de responsáveis existentes.')
      } else if (errorMessage.includes('already exists') || errorMessage.includes('unique')) {
        setFormError('Este email ou CPF já está cadastrado no sistema.')
      } else {
