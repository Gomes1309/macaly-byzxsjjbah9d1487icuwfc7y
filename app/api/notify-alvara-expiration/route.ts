...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const subjectPrefix = isExpired ? '📋 INFORMATIVO' : '📅 COMUNICADO'
    const actionRequired = isExpired 
      ? 'Estamos Renovando seu Alvará' 
...
<body>
    <div class="container">
        <div class="header">
            <h1>${isExpired ? '📋 ALVARÁ EM RENOVAÇÃO' : '📅 COMUNICADO DE VENCIMENTO'}</h1>
            <div class="subtitle">${urgencyMessage}</div>
            <div class="status-badge">${isExpired ? 'NOSSA EQUIPE ESTÁ CUIDANDO' : 'ACOMPANHAMENTO ATIVO'}</div>
        </div>
        ...
        <div class="content">
            ...
            <div class="alert-box">
                <h3 class="alert-title">
                    ${isExpired ? '📋 ALVARÁ VENCIDO - ESTAMOS CUIDANDO' : '📅 ALVARÁ PRÓXIMO DO VENCIMENTO'}
                </h3>
                <p style="margin: 0; font-size: 16px; line-height: 1.6;">
                    Informamos que o alvará da empresa <strong>${empresa}</strong> 
                    ${isExpired ? '<span class="urgent">está vencido</span>' : '<span class="warning">vencerá em breve</span>'}. 
                </p>
                <p style="margin: 15px 0 0 0; font-size: 16px; color: #059669; font-weight: 600;">
                    ✅ <strong>Fique tranquilo(a)!</strong> Nossa equipe da AG Assessoria já está providenciando 
                    a renovação e cuidando de todos os trâmites necessários.
                </p>
            </div>
            
            <!-- Seção de Tranquilidade -->
            <div style="background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); border-radius: 12px; padding: 25px; margin: 30px 0; border: 2px solid #22c55e;">
                <h4 style="color: #15803d; margin-top: 0; font-size: 20px; font-weight: 700; display: flex; align-items: center; gap: 10px;">
                    🛡️ Nossa Equipe Está Cuidando de Tudo
                </h4>
                <p style="margin: 15px 0; font-size: 16px; color: #166534; line-height: 1.6;">
                    Como sua empresa de confiança em gestão empresarial, já iniciamos os procedimentos 
                    para renovação do seu alvará. Você não precisa se preocupar com:
                </p>
                <ul style="color: #166534; margin: 15px 0; padding-left: 20px; line-height: 1.8;">
                    <li>📋 Documentação necessária</li>
                    <li>🏛️ Protocolo nos órgãos competentes</li>
                    <li>📞 Acompanhamento do processo</li>
                    <li>⏰ Prazos e renovações</li>
                </ul>
                <p style="margin: 15px 0 0 0; font-size: 16px; color: #15803d; font-weight: 600;">
                    💚 <strong>Resultado:</strong> Seu alvará será renovado sem que você precise se preocupar 
                    com burocracias ou prazos!
                </p>
            </div>
            ...
            <div class="contact-section">
                ...
                <p style="margin: 15px 0; font-size: 16px; color: #0c4a6e;">
                    Se desejar acompanhar o andamento do processo ou esclarecer dúvidas, 
                ...
                <p style="margin: 15px 0; font-size: 16px; color: #1e40af;">
                    ...
                    <span style="color: #64748b;">Seu Especialista em Licenciamento</span>
                </p>
                
                <div style="text-align: center; margin: 25px 0;">
                    <a href="https://wa.me/5516991098966?text=Olá! Gostaria de acompanhar o andamento da renovação do alvará da empresa ${encodeURIComponent(empresa)} (Protocolo: ${numeroProtocolo}). Obrigado!" 
            ...
            ${isExpired ? `
            <div style="background: #fff7ed; border: 2px solid #ea580c; border-radius: 8px; padding: 20px; margin: 25px 0; text-align: center;">
                <h4 style="color: #ea580c; margin-top: 0;">📋 INFORMAÇÕES IMPORTANTES</h4>
                <p style="color: #9a3412; margin: 10px 0; font-weight: 600;">
                    Como o alvará venceu há ${Math.abs(daysToExpire || 0)} dias, nossa equipe está 
                    trabalhando com prioridade máxima para evitar:
                </p>
                <ul style="color: #9a3412; text-align: left; margin: 15px 0; padding-left: 20px;">
                    <li>✅ Multas e penalidades (estamos cuidando)</li>
                    <li>✅ Problemas operacionais (em acompanhamento)</li>
                    <li>✅ Questões de fiscalização (monitoramento ativo)</li>
                    <li>✅ Complicações legais (prevenção em andamento)</li>
                </ul>
                <p style="color: #9a3412; margin: 15px 0 0 0; font-weight: 600;">
                    💚 <strong>Fique tranquilo:</strong> Estamos trabalhando para regularizar tudo rapidamente!
                </p>
            </div>
            ` : ''}
        </div>
        ...
        <div class="footer">
            ...
            <div class="footer-disclaimer">
                Este é um email informativo automático. Nossa equipe já está cuidando da renovação do seu alvará.<br>
                <strong>AG Assessoria em Gestão Empresarial</strong> - Cuidando da sua empresa com excelência e tranquilidade.
            </div>
        </div>
    </div>
</body>
    ...
    if (isProduction) {
      ...
      try {
        ...
        // Enviar email diretamente para o cliente usando o domínio personalizado
        const emailData = await resend.emails.send({
          from: 'AG Assessoria <noreply@agassessoriaonline.app.br>', // Usar domínio personalizado verificado
          to: [clienteEmail],
