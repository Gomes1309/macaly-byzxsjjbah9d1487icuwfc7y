...
import { Plus, Edit, Trash2, Search, Filter, Building, User, Mail, Phone, MapPin, Calendar, Check, X, Eye, Users, ArrowLeft, Download, Upload, RefreshCw, UserPlus, AlertCircle } from 'lucide-react'
...
import { CNPJFormData } from '@/hooks/useCNPJAutoComplete'
...
export default function GestaoClientesPage() {
  ...
  // Lidar com dados encontrados pelo CNPJ
  const handleCNPJDataFound = (cnpjData: CNPJFormData) => {
    console.log('Dados do CNPJ encontrados no formulário de clientes:', cnpjData)
    
    setFormData(prev => ({
      ...prev,
      nome: cnpjData.razaoSocial || prev.nome,
      email: cnpjData.email || prev.email,
      cpfCnpj: cnpjData.cnpj || prev.cpfCnpj,
      telefone: cnpjData.telefone || prev.telefone,
      endereco: cnpjData.logradouro && cnpjData.numero ? 
        `${cnpjData.logradouro}, ${cnpjData.numero} - ${cnpjData.bairro}, ${cnpjData.cidade}/${cnpjData.uf} - CEP: ${cnpjData.cep}` : 
        prev.endereco,
      tipoCliente: 'pessoa_juridica',
      observacoes: cnpjData.atividadePrincipal ? 
        `Atividade Principal: ${cnpjData.atividadePrincipal}${cnpjData.situacao ? ` | Situação: ${cnpjData.situacao}` : ''}` : 
        prev.observacoes
    }))
    
    toast.success('Dados do CNPJ preenchidos automaticamente!')
    console.log('Formulário atualizado com dados do CNPJ')
  }

  // Limpar dados do CNPJ
  const handleCNPJClear = () => {
    console.log('Limpando dados do CNPJ')
    setFormData(prev => ({
      ...prev,
      nome: '',
      email: '',
  ...
  const handleSave = async () => {
    try {
      ...
      const clienteData = {
        ...
        tipoCliente: formData.tipoCliente as 'pessoa_fisica' | 'pessoa_juridica',
      ...
      let result
      if (editingCliente) {
        result = await updateCliente(editingCliente.id, clienteData)
        toast.success('Cliente atualizado com sucesso!')
      } else {
        // Criar cliente com opções de conta
        result = await addCliente(clienteData, {
          createAccount: accountOptions.createAccount,
          cargo: accountOptions.cargo,
          notifyUser: accountOptions.notifyUser
        })
        
        if (result.account) {
          toast.success(result.message || 'Cliente e conta criados com sucesso!')
          if (accountOptions.notifyUser) {
            toast.info('Email com dados de acesso enviado para o cliente')
          }
        } else {
          toast.success(result.message || 'Cliente criado com sucesso!')
          if (result.error) {
            toast.warning('Cliente criado, mas houve erro ao criar conta de acesso')
          }
        }
      }
                ...
                <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
                  ...
                  <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                              <div className="space-y-0.5">
                                <Label htmlFor="createAccount">Criar conta de acesso automaticamente</Label>
                                <p className="text-sm text-gray-500">
                                  Cria uma conta de responsável para que o cliente possa acessar o sistema
                                </p>
                              </div>
                              <Switch
                                id="createAccount"
                            ...
                            {accountOptions.createAccount && (
                              <div className="space-y-3 pl-4 border-l-2 border-blue-200">
                                <div className="space-y-2">
                                  ...
                                  <Select value={accountOptions.cargo} onValueChange={(value) => setAccountOptions(prev => ({ ...prev, cargo: value }))}>
                                    <SelectTrigger>
                                      <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="space-y-0.5">
                                    <Label htmlFor="notifyUser">Notificar usuário por email</Label>
                                    <p className="text-sm text-gray-500">
                    ...
                    <div className="flex justify-end gap-2 sticky bottom-0 bg-white pt-4 border-t">
                      <Button variant="outline" onClick={() => setShowAddDialog(false)} className="text-gray-900">
                        Cancelar
                      </Button>
                      <Button 
                        onClick={handleSave}
