...
const loadFallbackData = async () => {
  try {
    ...
    // Para produção, tentar carregar dados reais da API primeiro
    try {
      ...
      if (result.success && result.data && result.data.responsavel) {
        ...
        console.log('Dados da API salvos com sucesso:', responsaveis)
        return responsaveis
      }
    } catch (apiError) {
      console.error('Erro ao carregar dados da API:', apiError)
      // Continuar com fallback local se API falhar
    }

    // Fallback local se API não funcionar
    ...
    console.log('Dados de fallback local salvos:', fallbackData)
    return fallbackData
...
export default function PortalClientePage() {
  ...
  const clearData = async () => {
    try {
      ...
      setDebugInfo('Dados limpos com sucesso!')
    } catch (error) {
  ...
  const handleLogin = async () => {
    try {
      ...
      if (responsavel.senha !== senha) {
        ...
        setError('Senha incorreta. Verifique sua senha ou use a opção "Esqueci meu PIN" abaixo.')
        return
      }
  ...
  const handleLoginSuccess = (responsavel: ResponsavelPortal | ResponsavelPF) => {
    ...
    if ('empresas' in responsavel && 'empresasIds' in responsavel) {
      ...
      responsavelProcessado = {
        ...responsavel,
        dataCriacao: responsavel.dataCriacao instanceof Date ? responsavel.dataCriacao : new Date(responsavel.dataCriacao),
        ultimoAcesso: responsavel.ultimoAcesso ? (responsavel.ultimoAcesso instanceof Date ? responsavel.ultimoAcesso : new Date(responsavel.ultimoAcesso)) : undefined,
        empresas: responsavel.empresas.map(empresa => ({
          ...empresa,
      ...
      const empresaFromPortal = {
        id: (responsavel as ResponsavelPortal).empresaId,
        cnpj: (responsavel as ResponsavelPortal).empresaCnpj,
        razaoSocial: (responsavel as ResponsavelPortal).empresaNome,
        nomeFantasia: (responsavel as ResponsavelPortal).empresaNome.includes('Ltda') ? (responsavel as ResponsavelPortal).empresaNome.replace(' Ltda', '') : undefined,
        email: responsavel.email,
        telefone: responsavel.telefone,
        endereco: 'Endereço não cadastrado',
        responsavelContabil: 'AG Assessoria Contábil',
        dataVinculacao: responsavel.dataCriacao instanceof Date ? responsavel.dataCriacao : new Date(responsavel.dataCriacao),
      ...
      responsavelProcessado = {
        ...
        senhaInicial: responsavel.senhaInicial,
        empresas: [empresaFromPortal],
        empresasIds: [(responsavel as ResponsavelPortal).empresaId],
        dataCriacao: responsavel.dataCriacao instanceof Date ? responsavel.dataCriacao : new Date(responsavel.dataCriacao),
        ultimoAcesso: responsavel.ultimoAcesso ? (responsavel.ultimoAcesso instanceof Date ? responsavel.ultimoAcesso : new Date(responsavel.ultimoAcesso)) : undefined
      }
    }
    ...
    console.log('Empresas do responsável:', responsavelProcessado.empresas.length)
    console.log('IDs de acesso:', responsavelProcessado.empresasIds)
    console.log('Empresas com acesso filtradas:', empresasComAcesso.length)
    console.log('Detalhes das empresas com acesso:', empresasComAcesso.map(e => ({
      id: e.id,
    ...
    if (empresasComAcesso.length === 1) {
      setCurrentCliente(empresasComAcesso[0])
      setIsAuthenticated(true)
      console.log('Login direto para empresa única:', empresasComAcesso[0].razaoSocial)
    } else {
  ...
  const handlePasswordRecovery = async () => {
    ...
    if (!recoveryData.cpf || !recoveryData.email) {
      setRecoveryError('Preencha CPF e email')
      return
    }
    
    console.log('Iniciando recuperação de senha:', recoveryData)
    
    try {
      await new Promise(resolve => setTimeout(resolve, 2000))
      
      // Normalizar CPF para comparação
      const cpfNormalizado = normalizeCPF(recoveryData.cpf)
      ...
      if (storedData) {
        try {
          ...
          console.error('Erro ao fazer parse dos dados de recuperação:', parseError)
        }
      }
      
      if (responsaveis.length === 0) {
        setRecoveryError('Nenhum usuário cadastrado encontrado. Entre em contato com o suporte.')
        return
      }
      
      // Verificar se CPF e email coincidem nos dados
      const responsavel = responsaveis.find(r => {
        const cpfResponsavelNormalizado = normalizeCPF(r.cpf)
        return cpfResponsavelNormalizado === cpfNormalizado && r.email === recoveryData.email
      })
      
      if (responsavel) {
        // Simular sistema de recuperação de senha mais realista
        // Gerar nova senha temporária
        const novaSenhaTemporaria = Math.random().toString(36).slice(-8).toUpperCase()
        ...
        const responsaveisAtualizados = responsaveis.map(r => {
          if (r.id === responsavel.id) {
            return {
              ...
              senhaInicial: true // Força troca de senha no próximo login
            }
          }
          return r
        })
        ...
        console.log(`Para: ${recoveryData.email}`)
        console.log(`Assunto: Recuperação de Senha - Portal do Cliente AG Assessoria`)
        console.log(``)
        console.log(`Olá ${responsavel.nome},`)
        console.log(``)
        console.log(`Recebemos uma solicitação de recuperação de senha para seu acesso ao Portal do Cliente.`)
        console.log(``)
        console.log(`Sua nova senha temporária é: ${novaSenhaTemporaria}`)
        console.log(``)
        console.log(`Por segurança, você será solicitado a criar uma nova senha personalizada no próximo login.`)
        console.log(``)
        console.log(`Se você não solicitou esta alteração, entre em contato conosco imediatamente.`)
        console.log(``)
        console.log(`Atenciosamente,`)
        console.log(`Equipe AG Assessoria Contábil`)
        console.log(`(16) 3987-3829`)
        console.log(`📧FIM SIMULAÇÃO EMAIL📧`)
        
        setRecoveryStep('sent')
        console.log('Email de recuperação enviado (simulado)')
        
        // Mostrar a senha no console para facilitar o teste
        console.log(`🔑 NOVA SENHA TEMPORÁRIA PARA TESTE: ${novaSenhaTemporaria}`)
        alert(`🔑 Para teste: Nova senha temporária é "${novaSenhaTemporaria}". Verifique o console para detalhes do "email".`)
        
      } else {
        setRecoveryError('CPF ou email não encontrados nos nossos registros')
      }
    } catch (error) {
      console.error('Erro na recuperação:', error)
      setRecoveryError('Erro interno. Tente novamente.')
    }
  }
  ...
  useEffect(() => {
    ...
    if (savedResponsavel && savedClient) {
      try {
        ...
        setIsAuthenticated(true)
        console.log('Autenticação automática:', clienteProcessado.razaoSocial)
      } catch (error) {
  ...
  // Carregar responsáveis do admin (forçar reload na inicialização)
  useEffect(() => {
    console.log('📖 Carregando responsáveis do admin...')
      ...
      if (savedResponsaveis) {
        try {
          ...
          if (!Array.isArray(data) || data.length === 0) {
            console.log('⚠️ Dados inválidos ou vazios no localStorage')
            return
          }
          
          // Verificar se os dados estão corrompidos (senha diferente da API)
          const hasCorruptedData = data.some((r: any) => 
            r.cpf === '218.680.918-48' && r.senha !== '1234' // Atualizado para PIN 1234
          )
          
          if (hasCorruptedData) {
            console.log('⚠️ Dados corrompidos detectados! Limpando localStorage...')
            localStorage.removeItem('portal_responsaveis')
            return
          }
          ...
          const validData = data.filter((r: any) => {
            const isValid = r.id && r.nome && r.cpf && r.senha && r.empresaNome
            if (!isValid) {
              console.log('⚠️ Responsável com dados inválidos encontrado:', r.nome || 'sem nome')
            }
            return isValid
          })
          
          if (validData.length !== data.length) {
            console.log(`⚠️ ${data.length - validData.length} responsáveis com dados inválidos foram filtrados`)
          }
          
          const responsaveisComDatas = validData.map((r: any) => ({
            ...r,
            dataCriacao: new Date(r.dataCriacao),
            ultimoAcesso: r.ultimoAcesso ? new Date(r.ultimoAcesso) : undefined
          }))
          
          console.log('✅ Responsáveis carregados do admin:', responsaveisComDatas.length)
          
          // Log das senhas para debug (remover em produção)
          console.log('🔑 CREDENCIAIS DISPONÍVEIS:')
          responsaveisComDatas.forEach((r, index) => {
            console.log(`👤 ${index + 1}. ${r.nome}`)
            console.log(`   CPF: "${r.cpf}" (normalizado: "${normalizeCPF(r.cpf)}")`)
            console.log(`   Senha: "${r.senha}"`)
            console.log(`   Email: "${r.email}"`)
            console.log(`   Primeira vez: ${r.senhaInicial}`)
            console.log(`   Empresa: ${r.empresaNome}`)
            console.log('   ────────────────────────────')
          })
        ...
        console.log('💡 Sistema será inicializado automaticamente quando necessário')
      }
    }
    ...
    // Recarregar dados a cada 10 segundos se estiver vazio (apenas 3 tentativas)
    let attemptCount = 0
    const maxAttempts = 3
    
    const interval = setInterval(() => {
      if (attemptCount >= maxAttempts) {
        console.log('🔄 Máximo de tentativas de recarregamento atingido')
        clearInterval(interval)
        return
      }
      
      const currentData = localStorage.getItem('portal_responsaveis')
      if (!currentData || JSON.parse(currentData).length === 0) {
        console.log(`🔄 Tentativa ${attemptCount + 1}/${maxAttempts}: Recarregando dados automaticamente...`)
        loadResponsaveis()
        attemptCount++
      } else {
        console.log('✅ Dados encontrados, parando tentativas automáticas')
        clearInterval(interval)
      }
    }, 10000)
    
    return () => {
      console.log('🧹 Limpando interval de recarregamento')
      clearInterval(interval)
    }
  ...
  useEffect(() => {
    if (isAuthenticated && currentCliente) {
      console.log('Carregando documentos reais para cliente:', currentCliente.id)
      ...
      if (savedDocumentos) {
        try {
          const parsedDocumentos = JSON.parse(savedDocumentos).map((doc: any) => ({
            ...
            // Converter tamanho de number para string se necessário
            tamanho: typeof doc.tamanho === 'number' ? `${(doc.tamanho / 1024 / 1024).toFixed(1)} MB` : doc.tamanho
          }))
          
          // Filtrar apenas documentos desta empresa (usando CNPJ normalizado)
          const cnpjNormalizado = currentCliente.cnpj.replace(/\D/g, '') // Remove formatação
          const documentosEmpresa = parsedDocumentos.filter((doc: DocumentoCliente) => {
            // Buscar cliente no localStorage para verificar o ID
            const savedClientes = localStorage.getItem('clientes_documentos')
            if (savedClientes) {
              try {
                const clientes = JSON.parse(savedClientes)
                const cliente = clientes.find((c: any) => c.cnpj.replace(/\D/g, '') === cnpjNormalizado)
                return cliente && doc.clienteId === cliente.id
              } catch (error) {
                console.error('Erro ao verificar cliente dos documentos:', error)
                return false
              }
            }
            return false
          ...
          console.log('Documentos encontrados para esta empresa:', documentosEmpresa.length)
          setDocumentos(documentosEmpresa)
        } catch (error) {
            ...
            <CardContent className="p-8 space-y-6">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-600 text-sm">
                  Entre com seu CPF e PIN numérico para acessar suas empresas
                </p>
                <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="flex items-center justify-center space-x-2 text-blue-700">
                    <Key className="w-4 h-4" />
                    <span className="text-sm font-medium">PIN de 4 dígitos - Simples e Seguro</span>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                ...
                {currentResponsavel?.empresas
                  ...
                  <Card 
                    key={empresa.id} 
                    className="cursor-pointer hover:shadow-xl transition-all duration-300 border-2 hover:border-blue-300 bg-gradient-to-br from-white to-blue-50/30"
                  ...
                  >
                    <CardContent className="p-6">
                      ...
                      <CardTitle className="text-lg font-semibold">{empresa.nomeFantasia || empresa.razaoSocial}</CardTitle>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-8">
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-6 text-white relative overflow-hidden">
            <div className="absolute top-0 right-0 opacity-20">
              <div className="w-32 h-32 bg-white rounded-full -mt-16 -mr-16"></div>
            </div>
            <div className="relative z-10">
              <h2 className="text-2xl font-bold mb-2">
                Bem-vindo, {currentCliente?.nomeFantasia || currentCliente?.razaoSocial}! 👋
              </h2>
              <p className="text-blue-100 mb-2">
                <strong>{currentResponsavel?.nome}</strong> • CPF: {currentResponsavel?.cpf ? formatCPF(currentResponsavel.cpf) : 'N/A'}
              </p>
              <p className="text-blue-100 mb-4">
                Aqui você encontra todos os documentos contábeis, obrigações e informações desta empresa.
              </p>
              <div className="flex items-center space-x-6 text-sm">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span>{currentResponsavel?.empresas.length || 1} Empresa{(currentResponsavel?.empresas.length || 1) > 1 ? 's' : ''}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-orange-600">{stats.obrigacoesPendentes}</p>
                </div>
        ...
        <Tabs defaultValue="documentos" className="space-y-6">
          <TabsList className="grid w-full grid-cols-4 bg-white border shadow-sm">
            ...
            <TabsTrigger value="obrigacoes" className="flex items-center space-x-2">
          ...
          <TabsContent value="documentos" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredDocumentos.map((documento) => {
                ...
                return (
                  <Card key={documento.id} className="bg-white hover:shadow-xl transition-all duration-300 border-0 shadow-lg">
                    <CardHeader className="pb-4">
                      ...
                      <CardTitle className="text-lg font-semibold">{documento.nome}</CardTitle>
                      <CardDescription className="text-sm text-gray-600">
                        {categoriaConfig[documento.categoria].label}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="flex justify-between text-sm text-gray-500">
                        <span>Tamanho: {documento.tamanho} KB</span>
                        <span>Upload: {documento.dataUpload.toLocaleDateString('pt-BR')}</span>
                      </div>
                      ...
                      {documento.dataVencimento && (
                        <div className="flex items-center space-x-2 text-sm">
                          ...
                          <span className="text-orange-600">
                            Vence: {documento.dataVencimento.toLocaleDateString('pt-BR')}
                          </span>
                        </div>
                      )}
                      ...
                      <Badge 
                        variant={documento.status === 'disponivel' ? 'default' : 
                                documento.status === 'enviado' ? 'secondary' : 'destructive'}
                        className="w-fit"
                      >
                        {documento.status}
                      </Badge>
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="max-w-2xl">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                {categoriaConfig[documento.categoria].label} • {documento.tamanho}
                              </DialogDescription>
                            </DialogHeader>
                            <div className="text-center py-12">
                              <FileText className="w-20 h-20 mx-auto mb-4 text-gray-300" />
                              <p className="text-gray-500 mb-2">Visualização não disponível</p>
                              <p className="text-sm text-gray-400">Faça o download para visualizar o documento</p>
                            </div>
                            {documento.observacoes && (
                              <div className="bg-gray-50 p-4 rounded-lg">
                                <h4 className="font-medium text-gray-900 mb-2">Observações:</h4>
                                <p className="text-gray-700 text-sm">{documento.observacoes}</p>
                              </div>
                            )}
                          </DialogContent>
                        </Dialog>
          ...
          <TabsContent value="obrigacoes" className="space-y-6">
            <div className="space-y-4">
              {obrigacoes.filter(o => o.clienteId === currentCliente?.id).map((obrigacao) => (
                <Card key={obrigacao.id} className="bg-white">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          <h4 className="text-lg font-semibold text-gray-900">{obrigacao.descricao}</h4>
                          <Badge 
                            variant={obrigacao.status === 'cumprida' ? 'default' : 
                                    obrigacao.status === 'vencida' ? 'destructive' : 'secondary'}
                          >
                            {obrigacao.status}
                          </Badge>
                        </div>
                        <div className="flex items-center space-x-4 text-sm text-gray-600">
                          <div className="flex items-center space-x-1">
                            <Calendar className="w-4 h-4" />
                            <span>Vencimento: {obrigacao.dataVencimento.toLocaleDateString('pt-BR')}</span>
                          </div>
                          <div className="flex items-center space-x-1">
                            <User className="w-4 h-4" />
                            <span>Responsável: {obrigacao.responsavel}</span>
                          </div>
                          {obrigacao.valor && (
                            <div className="flex items-center space-x-1">
                              <span>Valor: R$ {obrigacao.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                            </div>
                          )}
                        </div>
                        {obrigacao.observacoes && (
                          <p className="text-sm text-gray-500 mt-2">{obrigacao.observacoes}</p>
                        )}
                      </div>
                      <div className="text-right">
                        <div className="text-sm text-gray-500">
                          {Math.ceil((obrigacao.dataVencimento.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))} dias
                        </div>
                      </div>
                    </div>
                  </CardContent>
      ...
      <Dialog open={showPasswordRecovery} onOpenChange={setShowPasswordRecovery}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-600">
              {recoveryStep === 'input' && 'Digite seu CPF e email para recuperar sua senha'}
              {recoveryStep === 'sent' && 'Email de recuperação enviado com sucesso'}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    let value = normalizeCPF(e.target.value)
                    value = value.replace(/(\d{3})(\d)/, '$1.$2')
                    value = value.replace(/(\d{3})(\d)/, '$1.$2')
                    value = value.replace(/(\d{3})(\d{1,2})$/, '$1-$2')
                    setRecoveryData(prev => ({ ...prev, cpf: value }))
                  }}
                  placeholder="000.000.000-00"
                  maxLength={14}
                />
              ...
              {recoveryError && (
                <Alert className="bg-red-50 border-red-200">
                  <AlertCircle className="w-4 h-4 text-red-600" />
                  <AlertDescription className="text-red-800">{recoveryError}</AlertDescription>
                </Alert>
              )}
            </div>
          )}
          ...
          {recoveryStep === 'sent' && (
            <div className="py-4 text-center">
              ...
              <p className="font-medium text-blue-600 bg-blue-50 px-3 py-1 rounded-md inline-block mb-3">
                {recoveryData.email}
              </p>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="text-left">
                    ...
                    <ul className="text-sm text-yellow-700 space-y-1 list-disc list-inside">
                      ...
                      <li>Você será solicitado a criar uma nova senha personalizada</li>
            ...
            {recoveryStep === 'input' && (
              <div className="flex space-x-2 w-full">
                ...
                <Button 
                  ...
                  disabled={!recoveryData.cpf || !recoveryData.email}
                >
                  Enviar Email
                </Button>
              </div>
