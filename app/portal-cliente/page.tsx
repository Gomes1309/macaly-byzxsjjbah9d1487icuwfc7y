...
export default function PortalClientePage() {
    ...
    try {
      ...
      if (responsavel.senhaInicial) {
        ...
        const empresaFromPortal = {
          ...
          nomeFantasia: responsavelPortal.empresaNome.includes('LTDA') ? responsavelPortal.empresaNome.replace(' LTDA', '') : responsavelPortal.empresaNome,
  ...
  const handlePasswordRecovery = async () => {
    ...
    try {
      // Verificar se o email existe nos dados locais primeiro
      const storedData = localStorage.getItem('portal_responsaveis')
      ...
      if (storedData) {
        try {
          const responsaveis = JSON.parse(storedData)
          emailExists = responsaveis.some((r: any) => 
      ...
      const response = await fetch('/api/send-password-reset-email', {
        ...
        body: JSON.stringify({
          ...
          nome: 'Eduardo Aparecido Gomes', // Nome do usuário
          novaSenha: '22HHgYhJ' // Nova senha temporária
        })
      })
