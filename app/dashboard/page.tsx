...
export default function DashboardPage() {
  ...
  // Load dashboard data when authenticated - FIX: Remove dependency loop
  useEffect(() => {
    if (isAuthenticated) {
      loadDashboardData()
      loadPortalData() // Carregar dados do portal do cliente
      loadUsuariosData() // Carregar dados dos usuários
      loadDocumentosLocais() // Carregar dados locais dos documentos
    }
  }, [isAuthenticated]) // Remove dependencies to prevent infinite loop
  ...
  // Função para carregar dados dos Usuários do Sistema
  const loadUsuariosData = useCallback(() => {
    console.log('Carregando dados dos Usuários do Sistema...')
    ...
    try {
      ...
      console.log('Stats dos Usuários:', {
        usuariosAtivos,
        totalUsuarios,
        administradores,
        ultimoAcesso
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        alvaras: alvarasStats,
        abertura: {
          total: 0, // Placeholder - implementar hook de abertura se necessário
          emAndamento: 0,
          deferidos: 0,
          ultimaAtualizacao: new Date()
        },
      ...
      alvaras.slice(0, 2).forEach((alvara) => {
        activities.push({
          id: `alvara-${alvara.id}`,
          tipo: 'alvara',
          acao: 'Alvará registrado',
          descricao: `${alvara.empresa} - ${alvara.tipo}`,
          data: new Date(alvara.dataEmissao),
          status: alvara.status === 'em_dia' ? 'success' : 'warning'
        })
      })
      ...
      obrigacoes.slice(0, 2).forEach((obrigacao) => {
        activities.push({
          ...
          tipo: 'obrigacao',
          acao: obrigacao.status === 'cumprida' ? 'Obrigação cumprida' : 'Obrigação pendente',
          descricao: `${obrigacao.nomeObrigacao} - ${obrigacao.tipoObrigacao}`,
          data: obrigacao.dataCumprimento ? new Date(obrigacao.dataCumprimento) : new Date(obrigacao.dataVencimento),
          status: obrigacao.status === 'cumprida' ? 'success' : obrigacao.status === 'atrasada' ? 'warning' : 'info'
        })
      })
      
      // Add recent documentos activities
      documentos.slice(0, 2).forEach((doc) => {
        activities.push({
          id: `doc-${doc.id}`,
          tipo: 'documento',
          acao: 'Documento enviado',
          descricao: `${doc.nomeArquivo} - ${doc.tipoDocumento}`,
          data: new Date(doc.dataUpload),
          status: 'info'
        })
      })

      // Add recent documentos locais activities
      documentosLocais.slice(0, 2).forEach((doc) => {
        activities.push({
          id: `doc-local-${doc.id}`,
          tipo: 'documento',
          acao: 'Documento enviado (Local)',
      ...
      clientes.slice(0, 2).forEach((cliente) => {
        activities.push({
          ...
          descricao: `${cliente.nome} - ${cliente.cpfCnpj}`,
  ...
  }, [clientes, alvaras, documentos, obrigacoes, usuarios, documentosLocais, refreshClientes, refreshAlvaras, refreshDocumentos, refreshObrigacoes, refreshUsuarios, toast])
                ...
                <div className="flex items-center space-x-3">
                  <Button 
                    onClick={() => {
                      ...
                      loadDashboardData()
                      loadPortalData() // Também recarregar dados do portal
                      loadUsuariosData() // Também recarregar dados dos usuários
                      loadDocumentosLocais() // Também recarregar documentos locais
                    }}
                    variant="outline"
                    size="sm"
