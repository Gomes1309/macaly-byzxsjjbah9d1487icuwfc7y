...
export default function DashboardPage() {
  ...
  const { subscribe, forceGlobalRefresh, invalidateAllCaches } = useSyncManager()
  ...
  // Função para carregar dados locais dos documentos com validação
  const loadDocumentosLocais = useCallback(() => {
    ...
    try {
      ...
      if (savedDocumentos) {
        ...
        // NOVO: Filtrar documentos cujos clientes ainda existem no Supabase
        const validDocumentos = parsedDocumentos.filter((doc: any) => {
          if (!doc.clienteId) return false
          return clientes.some(cliente => cliente.id === doc.clienteId)
        })

        // Se houve filtração, atualizar localStorage
        if (validDocumentos.length !== parsedDocumentos.length) {
          localStorage.setItem('documentos_sistema', JSON.stringify(validDocumentos))
          console.log(`🧹 Removidos ${parsedDocumentos.length - validDocumentos.length} documentos órfãos`)
        }

        setDocumentosLocais(validDocumentos)
        console.log('📁 Documentos locais carregados:', validDocumentos.length)
        
        // Identificar clientes únicos dos documentos
        const clientesUnicos = new Set()
        validDocumentos.forEach((doc: any) => {
          if (doc.clienteId) {
  ...
  // Função para carregar dados do Portal do Cliente com validação
  const loadPortalData = useCallback(() => {
    ...
    try {
      ...
      if (savedResponsaveis) {
        ...
        // NOVO: Filtrar responsáveis cujas empresas/clientes ainda existem
        const validResponsaveis = responsaveis.map((resp: any) => {
          const validEmpresas = (resp.empresas || []).filter((emp: any) => 
            clientes.some(cliente => cliente.id === emp.id)
          )
          const validEmpresasIds = validEmpresas.map((emp: any) => emp.id)
          
          return {
            ...resp,
            empresas: validEmpresas,
            empresasIds: validEmpresasIds
          }
        }).filter((resp: any) => resp.empresas.length > 0) // Remove responsáveis sem empresas válidas

        // Se houve filtração, atualizar localStorage
        if (validResponsaveis.length !== responsaveis.length) {
          localStorage.setItem('portal_responsaveis', JSON.stringify(validResponsaveis))
          console.log(`🧹 Removidos ${responsaveis.length - validResponsaveis.length} responsáveis órfãos`)
        }
        
        // Calcular estatísticas
        const responsaveisAtivos = validResponsaveis.filter((r: any) => r.status === 'ativo').length
        const totalResponsaveis = validResponsaveis.length
        
        // Contar empresas únicas vinculadas
        const empresasUnicasSet = new Set()
        validResponsaveis.forEach((r: any) => {
  ...
  useEffect(() => {
      ...
      switch (event.type) {
        case 'clientes_updated':
          if (event.action === 'delete') {
            console.log('🧹 Dashboard: Cliente deletado, limpando dados relacionados')
            invalidateAllCaches()
            // Recarregar dados locais após limpeza
            setTimeout(() => {
              loadPortalData()
              loadDocumentosLocais()
            }, 500)
        ...
        case 'documentos_updated':
          if (event.action === 'delete') {
            console.log('🧹 Dashboard: Documento deletado, atualizando dados locais')
            loadDocumentosLocais()
          }
          break
        case 'global_refresh':
          console.log('🔄 Dashboard: Global refresh, recarregando tudo')
          refreshAllData()
          break
      }
    })
    ...
    return unsubscribe
  }, [subscribe, invalidateAllCaches, loadPortalData, loadDocumentosLocais, refreshAllData])
