...
import { Clock, FileText, Building, User, Users, LogOut, Shield, Settings, UserCheck, Star, UserPlus, Edit, Trash2, Search, Filter, MoreHorizontal, UserCog, Key, Activity, Database, Download, RefreshCw, Timer, Upload, Share, AlertTriangle, CheckCircle, Calendar, Menu } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const handleGenerateClientReport = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        link.href = url;
        link.download = `relatorio-clientes-${new Date().toISOString().split('T')[0]}.json`;
        link.click();
        URL.revokeObjectURL(url);
        
        toast({
          title: "✅ Relatório Gerado!",
          description: "O relatório de clientes foi gerado e baixado com sucesso.",
        });
      } else {
      ...
      toast({
        title: "❌ Erro",
        description: "Não foi possível gerar o relatório de clientes.",
        variant: "destructive",
      });
    } finally {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: `${result.data.metadata.totalRegistros} registros exportados com sucesso.`,
        });
      } else {
        throw new Error(result.error);
      }
      ...
      if (result.success) {
        ...
        if (logsWindow) {
          const logsHtml = `
            ...
            <html>
            <head>
              <title>Logs do Sistema - AG Assessoria</title>
              <style>
                body { font-family: Arial, sans-serif; margin: 20px; background: #f5f5f5; }
                .header { background: #333; color: white; padding: 15px; border-radius: 8px; margin-bottom: 20px; }
                .log-entry { background: white; padding: 10px; margin: 5px 0; border-radius: 5px; border-left: 4px solid #ccc; }
                .log-info { border-left-color: #2196F3; }
                .log-success { border-left-color: #4CAF50; }
                .log-warning { border-left-color: #FF9800; }
                .log-error { border-left-color: #F44336; }
                .timestamp { color: #666; font-size: 12px; }
                .level { font-weight: bold; padding: 2px 8px; border-radius: 3px; color: white; font-size: 11px; }
            ...
            <body>
              <div class="header">
                <h1>📋 Logs do Sistema - AG Assessoria</h1>
                <p>Gerado em: ${new Date().toLocaleString('pt-BR')}</p>
              </div>
              
              <div class="stats">
                <div class="stat">
                  <h3>${result.data.statistics.total}</h3>
                  <p>Total de Logs</p>
                </div>
                <div class="stat">
                  <h3>${result.data.statistics.info}</h3>
                  <p>Info</p>
                </div>
                <div class="stat">
                  <h3>${result.data.statistics.success}</h3>
                  <p>Success</p>
                </div>
                <div class="stat">
                  <h3>${result.data.statistics.warning}</h3>
                  <p>Warning</p>
                </div>
              </div>
              ...
              <div class="logs">
                ${result.data.logs.map((log: any) => `
                  <div class="log-entry log-${log.level.toLowerCase()}">
                    ...
                    <div style="color: #666; font-size: 12px; margin-top: 5px;">Usuário: ${log.user}</div>
                  </div>
                `).join('')}
              </div>
            </body>
        ...
        toast({
          ...
          description: `${result.data.statistics.total} logs do sistema carregados.`,
        });
      } else {
        throw new Error(result.error);
      }
  ...
  // Função para backup de usuários
  const handleBackupUsers = async () => {
    setIsBackupingUsers(true);
    try {
      const response = await fetch('/api/backup-users', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: `Backup de ${result.data.metadata.totalUsuarios} usuários gerado com sucesso.`,
        });
      } else {
        throw new Error(result.error);
      }
      ...
      toast({
        title: "❌ Erro",
        description: "Não foi possível fazer o backup dos usuários.",
        variant: "destructive",
      });
    } finally {
      setIsBackupingUsers(false);
    }
  };
  ...
  return (
    <div className={`min-h-screen p-4 transition-all duration-300 ${
            ...
            <div className="space-y-2">
              ...
              <div className={`p-3 rounded-lg border ${
                ...
                <div className="flex items-center justify-between">
                  ...
                  >
                    {isExportingData ? <RefreshCw className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3" />}
                  </Button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className={`p-3 rounded-lg border ${
                ...
                <div className="flex items-center justify-between">
                  ...
                  >
                    {isBackupingUsers ? <RefreshCw className="h-3 w-3 animate-spin" /> : <Database className="h-3 w-3" />}
                  </Button>
                </div>
              </div>
            </div>
      ...
      {/* Painel Solicitação de Abertura - NOVO PAINEL */}
      <div className="max-w-md mx-auto mb-6">
        <div className="bg-gradient-to-r from-rose-500 via-pink-500 to-purple-500 rounded-2xl p-[2px] shadow-xl">
          <div className={`rounded-2xl p-6 h-full transition-colors duration-300 ${
            isDarkMode ? 'bg-gray-800 text-white' : 'bg-white'
          }`}>
            <div className="text-center mb-4">
              {/* Ícone Solicitação */}
              <div className="relative inline-flex items-center justify-center w-16 h-16 mb-4">
                <div className="absolute inset-0 bg-gradient-to-r from-rose-400 to-purple-600 rounded-full animate-pulse opacity-20"></div>
                <div className="relative bg-gradient-to-r from-rose-500 to-purple-600 rounded-full p-3">
                  <FileText className="h-8 w-8 text-white" />
                </div>
              </div>
              
              <h2 className="text-xl font-bold bg-gradient-to-r from-rose-600 to-purple-600 bg-clip-text text-transparent mb-2">
                Solicitação Abertura
              </h2>
              <p className={`text-sm mb-4 ${
                isDarkMode ? 'text-gray-300' : 'text-gray-600'
              }`}>
                Formulário online para clientes solicitarem abertura de empresa
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              ...
              <button 
                onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/solicitacao-abertura`)
              ...
              <div className={`p-3 rounded-lg border ${
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button 
                    size="sm" 
                    variant="outline"
                    onClick={() => window.open('/solicitacao-abertura', '_blank')}
                    className={`text-xs ${
                      isDarkMode 
                        ? 'border-gray-600 text-gray-300 hover:bg-gray-600' 
