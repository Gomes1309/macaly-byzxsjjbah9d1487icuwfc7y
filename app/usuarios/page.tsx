...
export default function UsuariosPage() {
  ...
  const handleDeleteUser = async (userId: string, userName: string) => {
    console.log('🗑️ PÁGINA: Iniciando processo de exclusão:', { userId, userName })
    console.log('📊 PÁGINA: Estado antes da exclusão - Total usuários:', usuarios.length)
    
    if (window.confirm(`Tem certeza que deseja excluir o usuário "${userName}"?\n\nEsta ação não pode ser desfeita (exceto com o botão "Restaurar Excluídos").`)) {
      try {
        ...
        toast.error(`Erro ao excluir "${userName}": ${errorMessage}`)
      }
    } else {
      console.log('❌ PÁGINA: Usuário cancelou a exclusão')
    }
  }
