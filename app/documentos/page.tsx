...
const PASTAS = [
  {
    ...
    nome: 'Abertura/Alteração',
    descricao: 'Documentos de abertura e alteração empresarial',
    icone: Building,
    cor: 'blue'
  },
  {
    categoria: 'fiscal',
    nome: 'Fiscal',
    descricao: 'Documentos fiscais e tributários',
    icone: FileText,
    cor: 'green'
  },
  {
  ...
  },
  {
    ...
    nome: 'Trabalhista',
    descricao: 'Documentos trabalhistas e RH',
    icone: User,
    cor: 'orange'
  },
  {
    ...
    nome: 'Societário',
    descricao: 'Atas, contratos e documentos societários',
    icone: FileText,
    cor: 'red'
  },
  {
    categoria: 'juridico',
    nome: 'Jurídico',
    descricao: 'Procurações e documentos jurídicos',
    icone: FileText,
    cor: 'indigo'
  },
  {
...
export default function DocumentosPage() {
  ...
  // Integração com Gestão de Clientes (incluindo função de exclusão em cascata)
  const { clientes: clientesGestao, loading: loadingClientes, deleteCliente } = useClientes()
  ...
  const [showViewClienteDialog, setShowViewClienteDialog] = useState(false)
  const [showEditClienteDialog, setShowEditClienteDialog] = useState(false)
  const [showDeleteClienteDialog, setShowDeleteClienteDialog] = useState(false)
  ...
  // Load documentos do localStorage
  useEffect(() => {
    if (isAuthenticated) {
  ...
  // Edit cliente handler
  const handleEditCliente = useCallback(() => {
    if (!clienteToEdit) return
    
    console.log('Editing cliente:', clienteForm)
    ...
    setClientes(prev => prev.map(c => c.id === clienteToEdit.id ? clienteAtualizado : c))
    setClienteForm({
      nome: '',
      cnpj: '',
      email: '',
    ...
    toast({
      ...
      description: `${clienteAtualizado.nome} foi atualizado.`,
    })
  }, [clienteForm, clienteToEdit, toast])

  // Delete cliente handler (usando exclusão em cascata do hook useClientes)
  const handleDeleteCliente = useCallback(async () => {
    if (!clienteToDelete) return
    
    try {
      console.log('Iniciando exclusão em cascata do cliente:', clienteToDelete.nome)
      
      // Usar a função de exclusão em cascata do hook useClientes
      await deleteCliente(clienteToDelete.id)
      
      // Reset estado local após exclusão bem-sucedida
      if (clienteSelecionado?.id === clienteToDelete.id) {
        setClienteSelecionado(null)
        setCategoriaSelecionada(null)
      }
      
      // Remover documentos locais (já que a página usa estado local para documentos)
      setDocumentos(prev => prev.filter(doc => doc.clienteId !== clienteToDelete.id))
      
      setClienteToDelete(null)
      setShowDeleteClienteDialog(false)
      
      toast({
        title: "Cliente removido com sucesso!",
        description: `${clienteToDelete.nome} e todos os seus dados relacionados foram removidos do sistema.`,
      })
  ...
  }, [clienteToDelete, clienteSelecionado, deleteCliente, toast])

  // Open edit cliente dialog
  const openEditClienteDialog = useCallback((cliente: Cliente) => {
    ...
    setShowEditClienteDialog(true)
  }, [])

  // Open view cliente dialog
  const openViewClienteDialog = useCallback((cliente: Cliente) => {
    setClienteToView(cliente)
    setShowViewClienteDialog(true)
  }, [])

  // Open delete cliente dialog
  const openDeleteClienteDialog = useCallback((cliente: Cliente) => {
    setClienteToDelete(cliente)
    setShowDeleteClienteDialog(true)
  }, [])
  ...
  const handleUpload = useCallback(() => {
    ...
    const novosDocumentos = uploadFiles.map(file => ({
      ...
      arquivo: file,
      isShared: true, // Marcar como compartilhável por padrão
      shareToken: Date.now().toString() + Math.random().toString(36).substr(2, 9)
    }))
    
    setDocumentos(prev => [...prev, ...novosDocumentos])
    
    // Salvar também no formato que a API pode acessar (simulação)
    const documentosParaAPI = novosDocumentos.map(doc => ({
      id: doc.id,
      ...
      tipo: doc.tipo,
      tamanho: doc.tamanho,
      dataUpload: doc.dataUpload.toISOString(),
      uploadPor: doc.uploadPor,
    ...
    // Salvar referência para a API (simulação de cache)
    localStorage.setItem('documentos_shared_cache', JSON.stringify([
      ...JSON.parse(localStorage.getItem('documentos_shared_cache') || '[]'),
      ...documentosParaAPI
    ]))
    ...
    toast({
      ...
      description: `${novosDocumentos.length} documento(s) enviado(s) e habilitado(s) para compartilhamento.`,
    })
  ...
  const getFileIcon = (tipo: string) => {
    ...
    if (tipo.includes('excel') || tipo.includes('spreadsheet')) return <File className="w-5 h-5 text-green-500" />
  ...
  const handleShareDocument = useCallback(async (documento: Documento) => {
    console.log('Sharing document:', documento.nome)
    
    try {
      // Gerar URL de compartilhamento usando a nova API
      const response = await fetch(`/api/documents/share/${documento.id}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log('Link de compartilhamento gerado:', data.shareUrl)
        
        toast({
          title: "Link gerado com sucesso!",
          description: "O link de compartilhamento foi criado e está pronto para ser usado.",
        })
      } else {
      ...
      const shareableUrl = `${window.location.origin}/documents/share/${documento.id}`
      setShareUrl(shareableUrl)
      setSelectedDocument(documento)
      setShowShareDialog(true)
      
      toast({
        title: "Link gerado (modo compatibilidade)",
        description: "O link foi gerado usando o sistema local.",
        variant: "default",
      })
    }
  }, [toast])
  ...
  const shareViaEmail = useCallback(() => {
    ...
    const subject = `Documento: ${selectedDocument.nome} - AG Assessoria`
    const body = `Olá,
...
• Data de envio: ${format(new Date(), 'dd/MM/yyyy HH:mm')}
...
2. O documento será exibido automaticamente no seu navegador
3. Você pode visualizar online ou fazer download

Este link foi gerado automaticamente pelo sistema de documentos da AG Assessoria e permite acesso seguro ao documento compartilhado.

---
AG Assessoria Contábil
📞 (16) 99109-8966
📧 contato@ag-assessoria.com
🌐 ag-assessoria.com
...
Equipe AG Assessoria`
    ...
    console.log('Compartilhamento via email:', shareUrl)
    
    toast({
      title: "Email aberto",
      description: "O cliente de email foi aberto com a mensagem de compartilhamento.",
    })
  ...
  const shareViaWhatsApp = useCallback(() => {
    ...
    const message = `🔗 *Documento Compartilhado*\n\n` +
      ...
      `📌 *Como acessar:*\n` +
      `1. Clique no link acima\n` +
      `2. O documento será exibido no seu navegador\n` +
      `3. Você pode visualizar e fazer download\n\n` +
      `---\n` +
      `*AG Assessoria Contábil*\n` +
      `📞 (16) 99109-8966\n` +
      `📧 contato@ag-assessoria.com`
    ...
    console.log('Compartilhamento via WhatsApp:', shareUrl)
    
    toast({
      title: "WhatsApp aberto",
      description: "O WhatsApp foi aberto com a mensagem de compartilhamento.",
    })
  }, [selectedDocument, shareUrl, toast])

  // Converter clientes da Gestão de Clientes para formato do Documentos
  useEffect(() => {
    if (clientesGestao.length > 0) {
      console.log('Sincronizando clientes da Gestão de Clientes:', clientesGestao.length)
      const clientesConvertidos: Cliente[] = clientesGestao.map(cliente => ({
        id: cliente.id,
        nome: cliente.nome,
        cnpj: cliente.cpfCnpj, // conversão de cpfCnpj para cnpj
        email: cliente.email,
        telefone: cliente.telefone || '',
        endereco: cliente.endereco || '',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
          ...
          <Card className="bg-white shadow-lg border-0">
            <CardHeader className="bg-gradient-to-r from-slate-50 to-blue-50 rounded-t-lg">
              ...
              <CardDescription className="text-slate-600">
                📋 Clientes sincronizados da Gestão de Clientes • {clientes.length} cliente(s) • Selecione um para gerenciar documentos
              </CardDescription>
              {loadingClientes && (
      ...
      {/* View Cliente Dialog */}
      <Dialog open={showViewClienteDialog} onOpenChange={setShowViewClienteDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-slate-900 font-semibold">{clienteToView.nome}</p>
                </div>
                <div>
                  <Label className="text-slate-700 font-medium">CNPJ</Label>
                  <p className="text-slate-900">{clienteToView.cnpj}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-slate-900">{clienteToView.email}</p>
                </div>
                <div>
                  <Label className="text-slate-700 font-medium">Telefone</Label>
                  <p className="text-slate-900">{clienteToView.telefone}</p>
                </div>
              </div>
              
              <div>
                <Label className="text-slate-700 font-medium">Endereço</Label>
                <p className="text-slate-900">{clienteToView.endereco}</p>
              </div>
              
              <div>
                <Label className="text-slate-700 font-medium">Data de Cadastro</Label>
                <p className="text-slate-900">{format(clienteToView.dataCadastro, 'dd/MM/yyyy HH:mm')}</p>
              </div>
              
              <div>
                <Label className="text-slate-700 font-medium">Documentos</Label>
                <p className="text-slate-900">{documentos.filter(doc => doc.clienteId === clienteToView.id).length} documento(s) cadastrado(s)</p>
              </div>
              
              <div className="flex justify-end space-x-2 pt-4">
                <Button
                  variant="outline"
                  onClick={() => setShowViewClienteDialog(false)}
                  className="text-slate-600"
                >
                  Fechar
                </Button>
                <Button
                  ...
                  onClick={() => {
                    setShowViewClienteDialog(false)
                    openEditClienteDialog(clienteToView)
                  }}
                >
      ...
      {/* Edit Cliente Dialog */}
      <Dialog open={showEditClienteDialog} onOpenChange={setShowEditClienteDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
      ...
      <Dialog open={showDeleteClienteDialog} onOpenChange={setShowDeleteClienteDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Confirmar Exclusão</DialogTitle>
            <DialogDescription>
              Esta ação não pode ser desfeita. O cliente e todos os seus documentos serão removidos permanentemente.
            </DialogDescription>
          </DialogHeader>
          {clienteToDelete && (
            <div className="space-y-4">
              <div className="bg-red-50 p-4 rounded-lg border border-red-200">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="font-bold text-red-900">{clienteToDelete.nome}</h3>
                    <p className="text-sm text-red-700">{clienteToDelete.cnpj}</p>
                    <p className="text-sm text-red-700">{documentos.filter(doc => doc.clienteId === clienteToDelete.id).length} documento(s) será(ão) removido(s)</p>
                  </div>
                </div>
              </div>
      ...
      <Dialog open={showMultipleUpload} onOpenChange={setShowMultipleUpload}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Envie para uma empresa, todas empresas ou com detecção automática
            </DialogDescription>
          </DialogHeader>
          <div className="mt-4">
            <SimpleUpload />
