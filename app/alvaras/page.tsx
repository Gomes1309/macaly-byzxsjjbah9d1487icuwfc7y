...
export default function AlvarasPage() {
  ...
  const sendNotification = useCallback(async () => {
    ...
    try {
      console.log('📧 Enviando notificação de alvará para:', selectedAlvara.contato)
      
      // Calcular dias para vencimento
      const daysToExpire = getDaysToExpire(selectedAlvara.dataVencimento)
      
      // Determinar email e nome do destinatário
      let destinatarioEmail = selectedAlvara.contato
      let destinatarioNome = 'Cliente'
      
      // Se há cliente vinculado, usar os dados do cliente cadastrado
      if (selectedAlvara.clienteId) {
        const clienteVinculado = clientes.find(c => c.id === selectedAlvara.clienteId)
        if (clienteVinculado && clienteVinculado.email) {
          destinatarioEmail = clienteVinculado.email
          destinatarioNome = clienteVinculado.nome
          console.log('📧 Usando email do cliente cadastrado:', destinatarioEmail)
        } else {
          console.log('⚠️ Cliente vinculado sem email, usando contato do alvará:', destinatarioEmail)
        }
      }
      
      // Preparar dados para a API
      const notificationData = {
        clienteEmail: destinatarioEmail,
        clienteNome: destinatarioNome,
        empresa: selectedAlvara.empresa,
        cnpj: selectedAlvara.cnpj,
        tipo: getTypeLabel(selectedAlvara.tipo),
        numeroProtocolo: selectedAlvara.numeroProtocolo,
        dataVencimento: format(selectedAlvara.dataVencimento, 'dd/MM/yyyy'),
        status: selectedAlvara.status as 'vencendo' | 'vencido',
        responsavel: selectedAlvara.responsavel,
        daysToExpire: Math.abs(daysToExpire)
      }
      
      console.log('📤 Dados da notificação:', notificationData)
      
      // Chamar API real de notificação
      ...
      if (!response.ok) {
        // Verificar se é limitação da conta Resend
        if (result.testingMode && result.error?.includes('modo teste')) {
          toast({
            title: "⚠️ Limitação do Sistema de Email",
            description: `Email não enviado: ${result.message}`,
            variant: "destructive"
          })
          console.log('⚠️ Limitação Resend - Simulação:', result.simulatedEmail)
      ...
      toast({
        ...
        description: `Email enviado para ${destinatarioEmail} (${destinatarioNome}) sobre o alvará da empresa ${selectedAlvara.empresa}.`,
      })
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Erro ao enviar notificação. Tente novamente.",
        variant: "destructive"
      })
    } finally {
      setSendingNotification(false)
    }
  }, [selectedAlvara, toast, clientes])
  ...
  // Separar vencidos e vencendo para melhor controle no filtro
  const vencidosAlvaras = alvaras.filter(alvara => alvara.status === 'vencido')
  const vencendoAlvaras = alvaras.filter(alvara => alvara.status === 'vencendo')
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
          ...
          <div className="flex justify-between items-center h-20">
            <div className="flex items-center space-x-6">
              <div className="flex items-center space-x-4">
                <div className="bg-white p-3 rounded-xl shadow-lg">
                  <img 
                    src="https://assets.macaly-user-data.dev/vu6uteof0g2skav7mwkttz9h/mk2of4l6b3c7h8aflcpq3h6w/o-h2__8-krHhyceVQM4UJ/image.png" 
                    alt="AG Assessoria - Gestão Empresarial" 
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link href="/portal-cliente">
                <Button 
                  variant="outline"
                  className="text-purple-600 border-purple-300 hover:bg-purple-50 hover:border-purple-400 font-medium px-6 py-2 rounded-lg transition-colors duration-200"
                >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <Alert className="mb-6 border-orange-200 bg-orange-50">
            ...
            <AlertDescription className="text-orange-700">
              ...
              <Button 
                ...
                onClick={() => {
                  // Filtrar para mostrar primeiro vencidos, depois vencendo
                  const vencidos = alvaras.filter(alvara => alvara.status === 'vencido')
                  const vencendo = alvaras.filter(alvara => alvara.status === 'vencendo')
                  ...
                  // Scroll para a seção de alvarás após um breve delay
                  setTimeout(() => {
                    const alvarasSection = document.querySelector('[data-section="alvaras-list"]')
                    if (alvarasSection) {
                      alvarasSection.scrollIntoView({ behavior: 'smooth', block: 'start' })
                    }
                  }, 100)
                }}
              >
        ...
        {/* Alvarás List */}
        <Card className="bg-white shadow-lg border-0" data-section="alvaras-list">
          <CardHeader className="bg-gradient-to-r from-slate-50 to-blue-50 rounded-t-lg">
            <CardTitle className="text-blue-700 font-bold text-lg flex items-center justify-between">
              <span>Alvarás ({filteredAlvaras.length})</span>
              {statusFilter !== 'todos' && (
                <Button
                  variant="outline"
                  size="sm"
            ...
            <CardDescription className="text-slate-600">
              {statusFilter === 'vencido' ? 'Mostrando apenas alvarás vencidos' : 
               statusFilter === 'vencendo' ? 'Mostrando apenas alvarás vencendo' :
          ...
          {selectedAlvara && (
            <div className="space-y-4">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-orange-700 mt-1">
                      <strong>Email será enviado para:</strong> {
                        selectedAlvara.clienteId && clientes.find(c => c.id === selectedAlvara.clienteId)?.email 
                          ? `${clientes.find(c => c.id === selectedAlvara.clienteId)?.email} (Cliente: ${clientes.find(c => c.id === selectedAlvara.clienteId)?.nome})`
                          : `${selectedAlvara.contato} (Contato do alvará)`
                      }
                    </p>
                  </div>
                </div>
              ...
              <div className="text-sm text-slate-600">
                <p>
                  Uma notificação será enviada para <strong>
                    {selectedAlvara.clienteId && clientes.find(c => c.id === selectedAlvara.clienteId)?.email 
                      ? `${clientes.find(c => c.id === selectedAlvara.clienteId)?.email} (${clientes.find(c => c.id === selectedAlvara.clienteId)?.nome})`
                      : selectedAlvara.contato
                    }
                  </strong> informando sobre 
