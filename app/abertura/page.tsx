...
export default function DashboardAbertura() {
  ...
  const [editingProcesso, setEditingProcesso] = useState<ProcessoAbertura | null>(null)
  ...
  const generatePDFReport = async (title: string, data: ProcessoAbertura[], tipo: string) => {
    ...
    // Header with logo and enhanced design
    pdf.setFillColor(5, 150, 105) // emerald-600
    pdf.rect(0, 0, pageWidth, 40, 'F')
    
    // Company info with better spacing and typography
    pdf.setFontSize(18)
    ...
    pdf.text('ABERTURA / ALTERAÇÃO / BAIXA', 20, 28)
    
    pdf.setFontSize(11)
    pdf.text('CONTABILIDADE E ASSESSORIA EMPRESARIAL', 20, 35)
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.text('ABERTURA / ALTERAÇÃO / BAIXA - Contabilidade e Assessoria Empresarial', 20, pageHeight - 10)
    }
  ...
  // Handle checklist  
  const handleShowDocumentosNecessarios = () => {
    setShowDocumentosDialog(true)
  }

  // Generate documents PDF
  const generateDocumentsPDF = () => {
    const pdf = new jsPDF()
    const pageWidth = pdf.internal.pageSize.getWidth()
    ...
    pdf.setTextColor(0, 0, 0)
    pdf.setFont('helvetica', 'bold')
    pdf.text('DOCUMENTOS NECESSÁRIOS PARA ABERTURA DE EMPRESA', 20, 58)
    
    // Date with professional formatting
    pdf.setFontSize(10)
    pdf.setFont('helvetica', 'normal')
    pdf.setTextColor(100, 100, 100)
    pdf.text(`Gerado em: ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, 20, 68)
    
    // Documents list with enhanced styling
    let yPosition = 88
    pdf.setFontSize(14)
    pdf.setFont('helvetica', 'bold')
    pdf.setTextColor(0, 0, 0)
    pdf.text('DOCUMENTOS OBRIGATÓRIOS:', 20, yPosition)
    yPosition += 18
    
    const documentosObrigatorios = [
      'RG (Registro Geral) - Cópia autenticada',
      'CNH (Carteira Nacional de Habilitação) - Cópia autenticada', 
      'CPF (Cadastro de Pessoa Física) - Cópia autenticada',
      'Certidão de Casamento (se aplicável) - Cópia autenticada',
      'Comprovante de Endereço Residencial - Cópia simples',
      'Comprovante de Endereço Comercial - Cópia simples',
      'Cópia do IPTU do Local - Documento original ou cópia'
    ]
    ...
    documentosObrigatorios.forEach((doc, index) => {
      pdf.text(`${index + 1}. ${doc}`, 25, yPosition)
      yPosition += 8
    })
    ...
    pdf.setFontSize(14)
    pdf.setFont('helvetica', 'bold')
    pdf.text('DOCUMENTOS ADICIONAIS (Conforme atividade):', 20, yPosition)
    yPosition += 15
    
    const documentosAdicionais = [
      'Licença Ambiental (se necessário)',
      'Certificado de Condições Sanitárias (para alimentação)',
    ...
    pdf.setFont('helvetica', 'bold')
    pdf.setTextColor(0, 0, 0)
    pdf.text('CONTATO PARA DÚVIDAS:', 25, yPosition + 5)
    
    pdf.setFontSize(11)
    pdf.setFont('helvetica', 'normal')
    pdf.text('WhatsApp: (16) 99109-8966', 25, yPosition + 15)
    pdf.text('AG Assessoria - Contabilidade e Assessoria Empresarial', 25, yPosition + 25)
    
    // Footer
    pdf.setFontSize(8)
    pdf.setTextColor(128, 128, 128)
    pdf.text('ABERTURA / ALTERAÇÃO / BAIXA - Documentos necessários para abertura de empresa', 20, 280)
    pdf.text(`Página 1 de 1 - ${format(new Date(), 'dd/MM/yyyy')}`, pageWidth - 80, 280)
    
    // Save PDF
    const fileName = `documentos-necessarios-abertura-${format(new Date(), 'dd-MM-yyyy')}.pdf`
    pdf.save(fileName)
    
    toast({
      title: "PDF gerado com sucesso!",
      description: "Lista de documentos foi baixada.",
    })
  }
  ...
  const shareViaWhatsApp = () => {
    const documentos = [
      ...
      'Cópia do IPTU do Local'
    ]
    
    const message = `🏢 *DOCUMENTOS NECESSÁRIOS PARA ABERTURA DE EMPRESA*\n\n` +
      `📋 *DOCUMENTOS OBRIGATÓRIOS:*\n` +
      documentos.map((doc, i) => `${i + 1}. ${doc}`).join('\n') +
      `\n\n📞 *Dúvidas?*\n` +
      `Entre em contato conosco!\n` +
      `WhatsApp: (16) 99109-8966\n\n` +
      `🏛️ *AG Assessoria*\n` +
    ...
    toast({
      ...
      description: "Lista de documentos enviada para o WhatsApp.",
    })
  }
  ...
  console.log('✅ Funcionalidades implementadas: Editar, Excluir e Processo de Alteração de Empresa')
  console.log('🔘 Botões de ação agora visíveis tanto na aba Visão Geral quanto na aba Processos!')
  console.log('🔄 CARD UNIFICADO: Alteração e Baixa de empresa agora estão no mesmo card!')
  console.log('🗑️ EXCLUSÃO DEFINITIVA: Agora remove do localStorage E Supabase!')
    setIsLoading(true)
  ...
  // Edit processo handler
  const handleEditProcesso = useCallback((processo: ProcessoAbertura) => {
    console.log('Editando processo:', processo.id, processo.nomeEmpresa)
    setEditingProcesso(processo)
    
    // Populate form with processo data
    setFormData({
      ...
      cnaes: processo.cnaes.map(cnae => ({
        id: cnae.id,
        codigo: cnae.codigo,
        descricao: cnae.descricao,
        principal: cnae.principal
      })),
      capitalSocial: processo.capitalSocial.toString(),
      endereco: {
        ...processo.endereco,
        complemento: processo.endereco.complemento || ''
      },
      socios: processo.socios.map(socio => ({
        id: socio.id,
        nome: socio.nome,
        cpf: socio.cpf,
        rg: socio.rg,
      ...
      quantidadeFuncionarios: processo.quantidadeFuncionarios.toString(),
      prazoEstimado: format(processo.prazoEstimado, 'yyyy-MM-dd'),
      observacoes: processo.observacoes,
      prioridade: processo.prioridade,
      valorHonorarios: processo.valorHonorarios.toString(),
      valorTaxas: processo.valorTaxas.toString()
    })
    ...
    toast({
      ...
      description: `Editando processo: ${processo.nomeEmpresa}`,
    })
  }, [toast])

  // Save edited processo
  const handleSaveEditedProcesso = useCallback(() => {
    if (!editingProcesso) return
    
    console.log('Salvando edições do processo:', editingProcesso.id)
    
    const processoAtualizado: ProcessoAbertura = {
      ...editingProcesso,
      nomeEmpresa: formData.nomeEmpresa,
      razaoSocial: formData.razaoSocial,
      cnpj: formData.cnpj,
    ...
    const processosAtualizados = processos.map(p => 
      p.id === editingProcesso.id ? processoAtualizado : p
    )
    ...
    toast({
      ...
      description: `As alterações do processo ${processoAtualizado.nomeEmpresa} foram salvas.`,
    })
  }, [editingProcesso, formData, processos, toast])

  // Delete processo handler
  const handleDeleteProcesso = useCallback((processo: ProcessoAbertura) => {
    console.log('Selecionando processo para exclusão:', processo.id, processo.nomeEmpresa)
    setSelectedProcesso(processo)
  ...
  const confirmDeleteProcesso = useCallback(async () => {
    ...
    console.log('Confirmando exclusão do processo:', selectedProcesso.id, selectedProcesso.nomeEmpresa)
    console.log('🗑️ EXCLUSÃO DEFINITIVA: Removendo do localStorage E Supabase!')
    
    try {
      // Tentar excluir do Supabase (tabela empresas) se existir
      if (selectedProcesso.cnpj) {
        console.log('Tentando excluir do Supabase pela CNPJ:', selectedProcesso.cnpj)
        const { error: supabaseError } = await supabase
          .from('empresas')
          .delete()
          .eq('cnpj', selectedProcesso.cnpj)
        
        if (supabaseError) {
          console.log('Processo não encontrado no Supabase (normal para dados locais):', supabaseError.message)
        } else {
          console.log('✅ Processo excluído do Supabase com sucesso!')
        }
      }
      
      // Remover do estado local (localStorage)
      const processosAtualizados = processos.filter(p => p.id !== selectedProcesso.id)
      setProcessos(processosAtualizados)
      
      // Atualizar localStorage diretamente para garantir persistência
      localStorage.setItem('processos_abertura', JSON.stringify(processosAtualizados))
      console.log('✅ Processo excluído do localStorage com sucesso!')
      
      setShowDeleteDialog(false)
      setSelectedProcesso(null)
      
      toast({
        title: "Processo excluído definitivamente!",
        description: `O processo ${selectedProcesso.nomeEmpresa} foi removido permanentemente de todos os sistemas.`,
        variant: "destructive",
      })
      ...
      toast({
        ...
        description: "Ocorreu um erro ao tentar excluir o processo. Tente novamente.",
        variant: "destructive",
      })
    }
  }, [selectedProcesso, processos, toast])
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-blue-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
          ...
          <h1 className="text-lg font-bold text-slate-800 text-center mb-6">ABERTURA / ALTERAÇÃO / BAIXA</h1>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-blue-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Button 
                onClick={handleShowDocumentosNecessarios}
                variant="outline"
                className="text-emerald-600 border-emerald-300 hover:bg-emerald-50 hover:border-emerald-400 font-medium px-4 py-2 rounded-lg transition-colors duration-200"
              >
              ...
              <Button 
                ...
                variant="outline"
                className="text-purple-600 border-purple-300 hover:bg-purple-50 hover:border-purple-400 font-medium px-4 py-2 rounded-lg transition-colors duration-200"
              >
              ...
              <CardContent className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  >
                    ...
                    <span className="text-lg font-medium">Documentos</span>
                    <span className="text-sm opacity-90">Lista necessária</span>
                  </Button>
                  
                  {/* Card unificado para Alteração & Baixa */}
                  <div className="md:col-span-2 bg-gradient-to-r from-purple-50 to-red-50 border border-purple-200 rounded-lg p-6 space-y-4">
                    <div className="text-center mb-4">
                      <div className="flex items-center justify-center space-x-2 mb-2">
                        <Edit className="w-6 h-6 text-purple-600" />
                        <ArrowRight className="w-6 h-6 text-red-600" />
                      </div>
                      <h3 className="text-lg font-bold text-slate-800">Gestão de Empresas</h3>
                      <p className="text-sm text-slate-600">Alteração e baixa de empresas</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Button 
                        onClick={() => setShowAlteracaoDialog(true)}
                        variant="outline"
                        className="text-purple-600 border-purple-300 hover:bg-purple-50 p-4 h-auto flex flex-col items-center space-y-2"
                      >
                        <Edit className="w-6 h-6" />
                        <span className="text-sm font-medium">Alteração</span>
                        <span className="text-xs opacity-90">Modificar dados</span>
                      </Button>
                      
                      <Button 
                        onClick={() => setShowSelecionarProcessoBaixaDialog(true)}
                        variant="outline"
                        className="text-red-600 border-red-300 hover:bg-red-50 p-4 h-auto flex flex-col items-center space-y-2"
                      >
                        <ArrowRight className="w-6 h-6" />
                        <span className="text-sm font-medium">Baixa</span>
                        <span className="text-xs opacity-90">Encerrar empresa</span>
                      </Button>
                    </div>
                  </div>
              ...
              <CardContent className="p-6">
                {processos.slice(0, 3).map((processo) => (
                  <div key={processo.id} className="flex items-center space-x-4 p-4 bg-slate-50 rounded-lg mb-4 hover:bg-slate-100 transition-colors">
                    ...
                    {/* Action Buttons - Added for better visibility */}
                    <div className="flex items-center space-x-2">
                      <Button 
                        variant="outline" 
                        size="sm" 
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-blue-600">Editar Processo de Abertura</DialogTitle>
            <DialogDescription>
          ...
          <div className="flex justify-end space-x-2 pt-4">
            ...
            <Button
              onClick={handleSaveEditedProcesso}
          ...
          {selectedProcesso && (
            <div className="space-y-4">
              <div className="bg-red-50 p-4 rounded-lg border border-red-200">
                <h4 className="font-semibold text-red-800 mb-2">Processo a ser excluído:</h4>
                <p><strong>Nome:</strong> {selectedProcesso.nomeEmpresa}</p>
                <p><strong>Razão Social:</strong> {selectedProcesso.razaoSocial}</p>
                <p><strong>Status:</strong> {getStatusLabel(selectedProcesso.status)}</p>
              </div>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700">
                      Todos os dados relacionados a este processo serão perdidos permanentemente.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {/* Alteração de Empresa Dialog */}
      <Dialog open={showAlteracaoDialog} onOpenChange={setShowAlteracaoDialog}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-purple-600">Processo de Alteração de Empresa</DialogTitle>
            <DialogDescription>
              Selecione uma empresa e modifique os dados cadastrais necessários
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-purple-50 border-purple-200">
              <CardHeader>
                <CardTitle className="text-purple-700 text-lg">Selecionar Empresa para Alteração</CardTitle>
                <CardDescription>
                  Escolha qual empresa deseja alterar os dados cadastrais
                </CardDescription>
              </CardHeader>
              <CardContent>
                {processos.filter(p => p.status === 'deferido').length === 0 ? (
                  <div className="text-center py-8">
                    <div className="bg-slate-100 p-4 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                      <Building2 className="w-8 h-8 text-slate-400" />
                    </div>
                    <p className="text-slate-500">Nenhuma empresa disponível para alteração</p>
                    <p className="text-slate-400 text-sm mt-2">
                      Apenas empresas com status "Deferido" podem ser alteradas
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {processos.filter(p => p.status === 'deferido').map((processo) => (
                      <Card key={processo.id} 
                            className="border-purple-200 hover:border-purple-400 hover:shadow-md transition-all cursor-pointer"
                            onClick={() => {
                              setSelectedProcesso(processo)
            ...
            {selectedProcesso && (
              <Card className="bg-white border-slate-200">
                <CardHeader>
                  <CardTitle className="text-slate-700 text-lg">Dados para Alteração</CardTitle>
                  <CardDescription>
                    Modifique os campos necessários da empresa: {selectedProcesso.nomeEmpresa}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Tabs defaultValue="empresa" className="w-full">
                    ...
                    <TabsContent value="empresa" className="space-y-4 mt-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor="alter-nomeEmpresa">Nome da Empresa *</Label>
                          <Input
                            id="alter-nomeEmpresa"
                            value={formData.nomeEmpresa}
                            onChange={(e) => setFormData(prev => ({ ...prev, nomeEmpresa: e.target.value }))}
                            placeholder="Nome fantasia da empresa"
                            className="border-slate-300"
                        ...
                        <div>
                          <Label htmlFor="alter-razaoSocial">Razão Social *</Label>
                          <Input
                            id="alter-razaoSocial"
                            value={formData.razaoSocial}
                            onChange={(e) => setFormData(prev => ({ ...prev, razaoSocial: e.target.value }))}
                            placeholder="Razão social completa"
                            className="border-slate-300"
                        ...
                        <div>
                          <Label htmlFor="alter-capitalSocial">Capital Social *</Label>
                          <Input
                            id="alter-capitalSocial"
                            type="number"
                            step="0.01"
                            value={formData.capitalSocial}
                            onChange={(e) => setFormData(prev => ({ ...prev, capitalSocial: e.target.value }))}
                        ...
                        <div>
                          <Label htmlFor="alter-quantidadeFuncionarios">Quantidade de Funcionários *</Label>
                          <Input
                            id="alter-quantidadeFuncionarios"
                            type="number"
                            value={formData.quantidadeFuncionarios}
                            onChange={(e) => setFormData(prev => ({ ...prev, quantidadeFuncionarios: e.target.value }))}
                            placeholder="0"
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            <Label htmlFor="alter-cep">CEP *</Label>
                            <Input
                              id="alter-cep"
                              value={formData.endereco.cep}
                              onChange={(e) => setFormData(prev => ({ 
                          ...
                          <div className="md:col-span-2">
                            <Label htmlFor="alter-logradouro">Logradouro *</Label>
                            <Input
                              id="alter-logradouro"
                              value={formData.endereco.logradouro}
                              onChange={(e) => setFormData(prev => ({ 
                          ...
                          <div>
                            <Label htmlFor="alter-numero">Número *</Label>
                            <Input
                              id="alter-numero"
                              value={formData.endereco.numero}
                              onChange={(e) => setFormData(prev => ({ 
                          ...
                          <div>
                            <Label htmlFor="alter-complemento">Complemento</Label>
                            <Input
                              id="alter-complemento"
                              value={formData.endereco.complemento}
                              onChange={(e) => setFormData(prev => ({ 
                                ...prev, 
                                endereco: { ...prev.endereco, complemento: e.target.value }
                          ...
                          <div>
                            <Label htmlFor="alter-bairro">Bairro *</Label>
                            <Input
                              id="alter-bairro"
                              value={formData.endereco.bairro}
                              onChange={(e) => setFormData(prev => ({ 
                          ...
                          <div>
                            <Label htmlFor="alter-cidade">Cidade *</Label>
                            <Input
                              id="alter-cidade"
                              value={formData.endereco.cidade}
                              onChange={(e) => setFormData(prev => ({ 
                          ...
                          <div>
                            <Label htmlFor="alter-uf">UF *</Label>
                            <Input
                              id="alter-uf"
                              value={formData.endereco.uf}
                              onChange={(e) => setFormData(prev => ({ 
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="md:col-span-2">
                              <div className="flex items-center space-x-2">
                                <input
                                  type="checkbox"
                                  id={`admin-alter-${socio.id}`}
                                  checked={socio.administrador}
                                  onChange={(e) => updateSocio(socio.id, 'administrador', e.target.checked)}
                                  className="rounded border-slate-300"
                                />
                                <Label htmlFor={`admin-alter-${socio.id}`}>Administrador</Label>
                              </div>
                            </div>
                          </div>
                        </Card>
                    ...
                    <TabsContent value="cnaes" className="space-y-4 mt-6">
                      <div className="flex items-center justify-between">
                        <Label className="text-lg font-semibold">CNAEs da Empresa</Label>
                        <Button 
                          type="button" 
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="md:col-span-2">
                              <div className="flex items-center space-x-2">
                                <input
                                  type="checkbox"
                                  id={`principal-alter-${cnae.id}`}
                                  checked={cnae.principal}
                                  onChange={(e) => updateCNAE(cnae.id, 'principal', e.target.checked)}
                                  className="rounded border-slate-300"
                                />
                                <Label htmlFor={`principal-alter-${cnae.id}`}>Atividade Principal</Label>
                              </div>
                            </div>
                          </div>
                        </Card>
                    ...
                    <TabsContent value="contato" className="space-y-4 mt-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor="alter-email">Email da Empresa *</Label>
                          <Input
                            id="alter-email"
                            type="email"
                            value={formData.contato.email}
                        ...
                        <div>
                          <Label htmlFor="alter-telefone">Telefone da Empresa *</Label>
                          <Input
                            id="alter-telefone"
                            value={formData.contato.telefone}
                            onChange={(e) => setFormData(prev => ({ 
                        ...
                        <div>
                          <Label htmlFor="alter-celular">Celular da Empresa</Label>
                          <Input
                            id="alter-celular"
                            value={formData.contato.celular}
                            onChange={(e) => setFormData(prev => ({ 
                        ...
                        <div>
                          <Label htmlFor="alter-responsavel">Responsável *</Label>
                          <Input
                            id="alter-responsavel"
                            value={formData.responsavel}
                            onChange={(e) => setFormData(prev => ({ ...prev, responsavel: e.target.value }))}
                            placeholder="Nome do responsável"
                            className="border-slate-300"
                        ...
                        <div className="md:col-span-2">
                          <Label htmlFor="alter-observacoes">Observações sobre a Alteração</Label>
                          <Textarea
                            id="alter-observacoes"
                            value={formData.observacoes}
                            onChange={(e) => setFormData(prev => ({ ...prev, observacoes: e.target.value }))}
            ...
            {selectedProcesso && (
              <Button
                onClick={() => {
                  ...
                  // Criar um novo processo alterado
                  const processoAlterado: ProcessoAbertura = {
                    ...selectedProcesso,
                    nomeEmpresa: formData.nomeEmpresa,
                    razaoSocial: formData.razaoSocial,
                    capitalSocial: parseFloat(formData.capitalSocial) || 0,
                    ...
                    contato: formData.contato,
                    quantidadeFuncionarios: parseInt(formData.quantidadeFuncionarios) || 0,
                    observacoes: formData.observacoes + "\n\n[ALTERAÇÃO REALIZADA EM " + format(new Date(), 'dd/MM/yyyy HH:mm') + "]"
                  }
                  
                  // Atualizar a lista de processos
                  const processosAtualizados = processos.map(p => 
                    p.id === selectedProcesso.id ? processoAlterado : p
                  )
                  ...
                  toast({
                    ...
                    description: `Os dados da empresa ${processoAlterado.nomeEmpresa} foram atualizados.`,
                  })
                }}
                disabled={!formData.nomeEmpresa || !formData.razaoSocial || !formData.responsavel}
      ...
      {/* Documentos Necessários Dialog */}
      <Dialog open={showDocumentosDialog} onOpenChange={setShowDocumentosDialog}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-emerald-600 flex items-center space-x-2">
              <FileCheck className="w-6 h-6" />
              <span>Documentos Necessários para Abertura de Empresa</span>
            </DialogTitle>
            <DialogDescription>
              Lista completa de documentos obrigatórios e adicionais para abertura de empresa
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-emerald-50 p-6 rounded-lg border border-emerald-200">
              <h4 className="font-bold text-emerald-800 mb-4 flex items-center space-x-2">
                <CheckCircle2 className="w-5 h-5" />
                <span>Documentos Obrigatórios</span>
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {[
                  { doc: 'RG (Registro Geral)', obs: 'Cópia autenticada' },
                  { doc: 'CNH (Carteira Nacional de Habilitação)', obs: 'Cópia autenticada' },
                  { doc: 'CPF (Cadastro de Pessoa Física)', obs: 'Cópia autenticada' },
                  { doc: 'Certidão de Casamento', obs: 'Se aplicável - Cópia autenticada' },
                  { doc: 'Comprovante de Endereço Residencial', obs: 'Cópia simples' },
                  { doc: 'Comprovante de Endereço Comercial', obs: 'Cópia simples' },
                  { doc: 'Cópia do IPTU do Local', obs: 'Documento original ou cópia' }
                ].map((item, index) => (
                  <div key={index} className="flex items-start space-x-3 p-3 bg-white rounded-lg border border-emerald-100">
                    <div className="w-6 h-6 bg-emerald-600 text-white rounded-full flex items-center justify-center text-sm font-bold mt-0.5">
                      {index + 1}
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-slate-800">{item.doc}</p>
                      <p className="text-sm text-slate-600">{item.obs}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
              <h4 className="font-bold text-blue-800 mb-4 flex items-center space-x-2">
                ...
                <span>Documentos Adicionais (Conforme atividade)</span>
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {[
                  'Licença Ambiental (se necessário)',
            ...
            <div className="bg-gradient-to-r from-emerald-100 to-teal-100 p-6 rounded-lg border border-emerald-200">
              ...
              <div className="flex items-center space-x-2 text-emerald-700">
                ...
                <span className="font-medium">WhatsApp: (16) 99109-8966</span>
              </div>
              <p className="text-sm text-emerald-600 mt-2">
                AG Assessoria - Contabilidade e Assessoria Empresarial
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center pt-6 border-t">
            <Button
              variant="outline"
              onClick={() => setShowDocumentosDialog(false)}
              className="text-slate-600"
            >
              Fechar
            </Button>
